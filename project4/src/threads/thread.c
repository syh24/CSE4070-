...
//우선순위 비교
bool
priority_scheduling(const struct list_elem* left, const struct list_elem* right, void* aux) {
...
thread_set_priority (int new_priority) 
{
  ...
  //현재 priority가 가장 크지 않으면 다시 스케쥴링
  if (new_priority < get_max_priority()) thread_yield();
}
...
thread_set_nice (int nice UNUSED) 
{
  ...
  if (curr->priority < get_max_priority()) thread_yield();
}
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  sema_init(&(t->lock_sema), 0);
  sema_init(&(t->sync_sema), 0);
  list_init(&(t->child_list));
  list_push_back(&(running_thread()->child_list), &(t->child_elem_list));
...
void check_wake_up_thread (int64_t ticks) 
{
  struct thread *t;
	struct list_elem *e;
...
{
  struct list_elem *e;
  //running thread의 recent_cpu값을 1 증가.
  ...
  if (timer_ticks() % TIMER_FREQ == 0) {
    ...
    cal_load_avg(ready_threads);

    for (e = list_begin(&all_list); e != list_end(&all_list); e = list_next(e)) {
      struct thread *t = list_entry(e, struct thread, allelem);
      if (t != idle_thread) cal_recent_cpu(t);
    }
  }
...
void cal_recent_cpu(struct thread *t) {
  int e1 = int_mul_float(2, load_avg);
  int e2 = float_add_int(e1, 1);
  int e3 = float_div_float(e1, e2);
  int e4 = float_mul_float(e3, t->recent_cpu);
  t->recent_cpu = float_add_int(e4, t->nice);
}

void cal_load_avg(int rt_cnt) {
  int e1 = int_mul_float(59, load_avg);
  int e2 = float_add_int(e1, rt_cnt);
  load_avg = float_div_int(e2, 60);
}

void cal_priority(struct thread *t) {
  int e1 = float_div_int(t->recent_cpu, 4);
  int e2 = t->nice * 2;
  int e3 = int_sub_float(PRI_MAX - e2, e1);
  t->priority = e3 / FRACTION;
