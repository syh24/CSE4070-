...
process_execute (const char *file_name) 
{
  ...
  for (e = list_begin(&thread_current()->child_list); e != list_end(&thread_current()->child_list); e = list_next(e)) {
    t = list_entry (e, struct thread, child_elem_list);
...
start_process (void *file_name_)
{
  ...
  sema_up(&thread_current()->parent->sync_sema);
...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  for (e = list_begin(&curr->child_list); e != list_end(&curr->child_list); e = list_next(e)) {
    t = list_entry (e, struct thread, child_elem_list);
    if(child_tid == t->tid) {
        sema_down(&t->lock_sema);
        status = t->exit_status;
        list_remove(&t->child_elem_list);
        sema_up(&t->free_sema);
        // printf("%d", status);
        return status;
    }
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  strlcpy(fn_copy, file_name, strlen(file_name) + 1);
	tmp = strtok_r(fn_copy, " ", &next_ptr);

  //argv에 command parsing
	while(tmp != NULL) {
    argv[argc] = tmp;
...
setup_stack (void **esp) 
{
  ...
  if (kpage != NULL) 
  {
    success = install_page (pg_round_down(vaddr), kpage->kaddr, true);
    if (success) {
      *esp = PHYS_BASE;
    }
    else {
  ...
  // printf("after alloc setup_stack");

  struct pte *pte = malloc(sizeof(struct pte));
  pte->uaddr = pg_round_down(vaddr);
  pte->loaded = true;
  pte->writable = true;
  pte->dirty_bit = false;
  pte->is_second_chance = false;
